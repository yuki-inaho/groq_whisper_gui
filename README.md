...
| クリップボードへ手動コピー | 「クリップボードへコピー」ボタンで直前の転写を任意のタイミングで再コピーできます。 | 自動反映を取りこぼした場合にも使えます。 |
| IME 非依存ショートカット (Linux) | IME が全角入力モードでもショートカットが動作します。 | `/dev/input` を読む evdev を利用。`input` グループ所属が必要です（後述）。 |
...
| `input` グループ所属 (Linux) | 任意 | IME 全角モードでもショートカットを効かせる evdev 入力に必要です。未所属でも半角入力時はショートカットが動作します。 |
...
10. 「クリップボードへコピー」ボタンで、直前の文字起こし結果を任意のタイミングでクリップボードへ再コピーできます。

## IME 全角入力モードでのショートカット (Linux)

Linux では IME (ibus / fcitx 等) が全角入力モード時にキー入力を横取りし、`Space` / `Esc` などのショートカットが効かないことがあります。本アプリは `/dev/input` をカーネル層で直接読む evdev により、IME の全角・半角に関わらずショートカットを動作させます（アプリにフォーカスがある時のみ反応します）。

この機能には `input` グループへの所属が必要です。

```bash
sudo usermod -aG input "$USER"
```

反映には再ログインが必要です。再ログインせずに試す場合は、`input` グループを付与して起動します。

```bash
newgrp input
# もしくは、その場限りで起動する場合（sg が ast-grep 等と衝突する環境では絶対パスを使用）:
/usr/bin/sg input -c 'groq-whisper-app'
```

`input` グループに所属していない場合でも、半角入力時は従来どおりショートカットが動作します。起動時のデバッグログ（`--ui-mode debug`）に evdev の有効・無効が出力されます。
